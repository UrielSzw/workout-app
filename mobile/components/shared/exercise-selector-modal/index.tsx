...
export const ExerciseSelectorModal: React.FC<Props> = ({
  ...
  const renderExerciseCard = useCallback(
    ...
    [isExerciseSelected, onSelectExercise, colors, exerciseModalMode],
  );
  ...
  return (
    ...
    >
      <View style={{ flex: 1, backgroundColor: colors.background }}>
        ...
        >
          {exerciseModalMode === 'add-new' && selectedExercises.length > 0 && (
            <View style={{ gap: 12 }}>
          ...
          {exerciseModalMode === 'replace' && selectedExercises.length > 0 && (
          ...
          {exerciseModalMode === 'add-to-block' && (
            <Button variant="primary" fullWidth onPress={onAddToBlock}>
              Agregar {selectedExercises.length} ejercicio
              {selectedExercises.length > 1 ? 's' : ''} al bloque
            </Button>
          )}
        </View>
      </View>
