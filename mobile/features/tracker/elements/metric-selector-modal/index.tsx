...
import { PREDEFINED_METRICS } from '../../mock';
...
export const MetricSelectorModal: React.FC<Props> = ({ visible, onClose }) => {
  ...
  const metrics = useMetricState();
  const { addMetric } = useMetricActions();

  const metricsSlugs = Object.values(metrics).map((metric) => metric.slug);

  const inactiveMetrics = PREDEFINED_METRICS.filter(
    (metric) => !metricsSlugs.includes(metric.slug || ''),
  );

  const handleToggleMetric = (metric: IMetricDefinition) => {
    addMetric(metric);
  };
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Typography variant="caption" color="textMuted">
                      {metric.defaultTarget
                        ? `Meta: ${metric.defaultTarget} ${metric.unit}`
                        : `Unidad: ${metric.unit}`}
                    </Typography>
                  </View>
